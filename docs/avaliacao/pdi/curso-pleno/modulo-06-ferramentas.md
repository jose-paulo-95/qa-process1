...
**Objetivo:** Ao final deste módulo, o aluno será capaz de usar Git (branch, merge, conflitos), Jira (JQL, dashboards), analisar logs para debugging, e aplicar métricas e planejamento de testes.
...
git push origin feature/x  # Enviar
git merge main             # Mesclar main na branch atual
```

**Conflitos:** Ao fazer merge, se houver conflito, abra os arquivos marcados, resolva manualmente (remova marcadores <<<<, ====, >>>>), depois git add e git commit.

**Cheat sheet:** Crie um arquivo pessoal com os comandos que você mais usa.
